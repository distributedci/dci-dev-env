...
Generate a random value using an `UUID`, then we will use it for the `nonce` field in the following link:
    ...
    http://localhost:8180/auth/realmsredhat-external/protocol/openid-connect/auth?\
    client_id=dci-cs&\
...
From the CLI:
    ...
    $ curl \
      ...
      http://localhost:8180/auth/realmsredhat-external/protocol/openid-connect/token
