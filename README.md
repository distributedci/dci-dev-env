...
For instance
    ...
    podman exec -it dci-dev-env_api_1 bash
...
You can initialize or re-initialize the database from the API container by running:
    ...
    podman exec -it dci-dev-env_api_1 ./bin/dci-dbprovisioning
...
To run `tox` on dci-control-server:
    ...
    podman exec -it -w /opt/dci-control-server dci-dev-env_tox_1 tox

To run `tox` on python-dciclient:

    podman exec -it -w /opt/python-dciclient dci-dev-env_tox_1 tox

To run `tox` on dci-ansible:

    podman exec -it -w /opt/dci-ansible dci-dev-env_tox_1 tox
